...
void Innerkernel(int m, int n, int k, double *a, int lda, double *b, int ldb, double *c, int ldc, int first_time, double *packedB)
{
  int i, j;
  double packedA[m*k];
                                    ...
                                    double *c, int ldc )
{
  ...
  for ( j=0; j< k ; j+=kc ){        /* Loop over the columns of C */
    ...
    double *packedB;
    packedB = ( double * ) malloc( kc*nb * sizeof( double ) );  
    
    for ( i=0; i<m; i+=mc ){        /* Loop over the rows of C */
      /* Update the C( i,j ) with the inner product of the ith row of A
	 and the jth column of B */
      ib = min(m-i, mc);
      Innerkernel(ib, n, jb, &A(i, j), lda, &B(j,0), ldb, &C(i, 0), ldc, i==0, packedB);
